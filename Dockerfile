...
ENV JS_EE_VERSION 6.2.0
ENV JS_EE_HOME /opt/jasperreports-server-${JS_EE_VERSION}
ENV PATH $PATH:${JS_EE_HOME}

RUN apt-get update && apt-get install -y wget \
	&& wget --progress=bar:force:noscroll -O jasperreports-server-linux-x64-installer.run https://d2ev2buidpvgfo.cloudfront.net/${JS_EE_VERSION}/Installers/jasperreports-server-${JS_EE_VERSION}-linux-x64-installer.run \
